...
- COS: `COS_BUCKET`, `COS_REGION`, `COS_SECRET_ID`, `COS_SECRET_KEY`, `PUBLIC_ASSET_BASE_URL`
...
Rotate ZPAY, TMS, and COS keys by deploying new secrets first, then smoke-testing payment creation, TMS approval, upload URL creation, and refund submission in a staging environment.

## Evidence Storage

COS upload URLs are generated by `POST /api/admin/expenses/evidence/upload-url`. Uploaded objects must be readable through `PUBLIC_ASSET_BASE_URL` before the resulting URL is attached to an expense record.
